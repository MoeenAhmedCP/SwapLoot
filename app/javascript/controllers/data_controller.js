...
export default class extends Controller {
  ...
  triggerBuying = (event) => {
    if (event.target.checked) {
      ...
      this.unsubscribe(event.target.id)
    }
  };

  subscribeEventsChannel = async (accountId) => {
    consumer.subscriptions.create({ channel: "CsgosocketChannel", channel_id: accountId }, {
      connected() {
      ...
      async fetchCsGoEmpireData(accountId) {
        console.log("fetchCsGoEmpireData", accountId)
      ...
      async fetch_user_data(accountId) {
        ...
        console.log("fetch_user_data accountId", accountId)
  ...
  unsubscribe(accountId) {
    const subscription = consumer.subscriptions.subscriptions.find(subscription => {
