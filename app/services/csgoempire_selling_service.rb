class CsgoempireSellingService 
  ...
  def price_cutting_down_for_listed_items
    ...
    response = HTTParty.get(BASE_URL + '/trading/user/trades', headers: headers)
    ...
    items_listed_for_sale.each do |item|
      if updated_at_hours_from_now?(item["updated_at"], 12) && item["auction_number_of_bids"] == 0
        ...
        response = HTTParty.post(BASE_URL + "/trading/deposit/#{item["deposit_id"]}/cancel", headers: headers)
        puts response.code == 200 ? "#{item["market_name"]}'s deposit has been cancelled." : "Something went wrong with #{item["item_id"]} - #{item["market_name"]} Unable to Cancel Deposit."
  ...
  def updated_at_hours_from_now?(updated_at, no_of_hours)
  ...
  def deposit_items_for_sale(items)
    ...
    body = {items: items}
    response = HTTParty.post(BASE_URL + '/trading/deposit', headers: headers, body: body.to_json)
    if response.code == 200
      result = JSON.parse(response.body)
      puts "Item Listed for sale on CSGO Empire."
    else
      result = "API request failed"
      puts "Error !!! Item can not Listed for sale on CSGO Empire."
    end
    result
  end
