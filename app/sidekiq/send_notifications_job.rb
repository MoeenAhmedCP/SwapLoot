class SendNotificationsJob
  ...
  def perform(user_id, notification_type, steam_account_id, item_market_name, item_id, item_total_value)
    p "<=========== Notification sending ===================>"
    current_user = User.find(user_id)
    @notification = current_user.notifications.create(title: "Item #{notification_type}", body: "#{item_market_name} #{notification_type} with ID: (#{item_id}) at price (#{(item_total_value.to_f)/100}) coins", notification_type: notification_type)
    ActionCable.server.broadcast("flash_messages_channel_#{current_user.id}", { message: "#{item_market_name} #{notification_type} with ID: (#{item_id}) at price (#{(item_total_value.to_f)/100}) coins" })
    ...
    if notification_type == "Sold"
      ...
      NewInventoryItemJob.perform_in(15.seconds, item_market_name, item_total_value, steam_account_id)
    end
  end
