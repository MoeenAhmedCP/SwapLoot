class NewInventoryItemJob
  ...
  def perform(item_market_name, item_market_value, steam_account_id)
    p "<=========== Add Inventory Job ===================>"
    ...
    matching_items = items_not_in_inventory.select { |api_item| api_item['market_name'] == item_market_name }
    matching_items.each do |items|
      ...
      inventory.update(market_price: (item_market_value * 0.614))
    end
  end
end
